...
const CATEGORIES = ["All", "Pizza", "Biryani", "Burger", "Chinese", "South Indian", "Desserts", "Nightlife", "Dining", "Cafe", "Healthy"];

const RestaurantList = ({ onSelectRestaurant, searchQuery, activeFilter, filterLabel, onClearFilter }) => {
  ...
  const filtered = useMemo(() => {
    ...
    // External filter from Card/Collection/Cities click
    if (activeFilter && activeFilter !== "All") {
      if (activeFilter === "order-online") {
        list = list.filter((r) => r.category.includes("order-online"));
      } else if (activeFilter === "locality") {
        // filterLabel is the locality name
        list = list.filter((r) => r.locality === filterLabel);
      } else {
        list = list.filter((r) => r.category.includes(activeFilter));
      }
    }
    ...
    if (fastDelivery) list = list.filter((r) => r.delivery > 0 && r.delivery <= 25);
    ...
    return list;
  }, [searchQuery, activeCategory, vegOnly, fastDelivery, topRated, sortBy, activeFilter, filterLabel]);
    ...
    <div className="restaurant-list">
      ...
      {activeFilter && activeFilter !== "All" && (
        <div className="filter-banner">
          <span>
            {activeFilter === "locality" ? `📍 ${filterLabel}` : `🔍 ${filterLabel}`}
          </span>
          ...
          {filtered.map((r) => (
            <div className="restaurant-card" key={r.id} onClick={() => onSelectRestaurant(r)}>
              ...
              <div className="card-body">
                ...
                <div className="card-locality">📍 {r.locality}</div>
                <div className="card-meta">
                  <span className="rating">★ {r.rating} ({r.reviews.toLocaleString()})</span>
                  <span className="delivery-time">
                    {r.delivery > 0 ? `⏱ ${r.delivery} mins` : "🏠 Dine-in only"}
                  </span>
